...
export abstract class GeneratedPaisesService extends BaseService<Paises> implements OnInit {
  ...
  override guid(paises: Paises) {
    paises.id = Math.floor((1 + Math.random()) * 0x10000);
  }
  ...
  override deleteEntity(
    ...
    if (!this.memory) {
      const filterAux = this.mapFilterForDelete(id);
      return this.http
  ...
  override getEntity(id: string): Observable<Paises> {
    if (!this.memory) {
      const filterAux = this.mapFilterForGet(id);
      return this.http
        .post<Paises>(
          this.editUrl,
          filterAux,
  ...
  mapFilterForDelete(id: any) {
    return {
      id: id
    };
  }  

  mapFilterForGet(id: any) {
    return {
      id: id
    };
  }  

  mapFilterForUpdate(id: any) {
    return {
      id: id
    };
  }    
